    ...
    <>
      ...
      <main className="mx-4 md:mx-[400px] bg-inherit md:shadow-lg">
        ...
        <form action="">
          ...
          <div className="mt-6">
            <label htmlFor="password" className="pb-4">Password (6+ characters)</label> <br />
            <input type={showPassword ? 'text' : 'password'} name="" id="password" className="relative border-[1px] border-slate-500 w-full p-3 rounded-sm hover:transition hover:bg-gray-100 hover:outline hover:outline-1" />
            <span className="absolute md:right-[27rem] right-8 translate-y-4 text-[#0A66c2] font-bold text-sm">
              <button onClick={() => setShowPassword(!showPassword)} type="button">{showPassword ? 'Hide' : 'Show'}</button>
            </span>
          </div>
          <div className="mt-8">
        ...
        <div>
          ...
          <div className="mt-8 relative">
            <button style={{ width: '100%', borderRadius: '30px', padding: '10px 24px' }} className="relative bg-transparent border-[1px] border-slate-300 text-neutral-600 text-md font-normal hover:transition hover:bg-gray-100">
          ...
          <div className="mt-4">
            <button style={{ width: '100%', borderRadius: '30px', padding: '10px 24px' }} className="relative bg-transparent border-[1px] border-slate-300 text-neutral-600 text-md font-normal hover:transition hover:bg-gray-100">
              <span className="absolute -translate-x-8"><Image src={appleImage} sizes="10vw" alt="google icon"></Image></span>
              Sign in with Apple
            </button>
          </div>
          <div className="mt-4">
            <button style={{ width: '100%', borderRadius: '30px', padding: '10px 24px' }} className="bg-transparent border-[1px] border-slate-300 text-neutral-600 text-md font-normal hover:transition hover:bg-gray-100">
              <span className="absolute -translate-x-8"><Image src={oneTimeSignin} sizes="10vw" alt="google icon"></Image></span>
              Sign in with one-time link
            </button>
          </div>
          <div className="mt-8">
            <p className="text-center">New to Linkedin? <Link href="/signup" className="text-[#0A66c2] font-normal active:bg-blue-50 active:rounded-lg active:underline">Join now</Link></p>
          </div>
        </div>
      </main>
    </>
