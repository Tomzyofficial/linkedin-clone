...
export default function CourseListing() {
  const renderedCourses = coursesData.map((course) => (
    <li key={course.id}>
      <Link href="#" className="hover:underline text-[#000000E6] block">
        {course.course} <br />
        <span className="text-slate-500">{course.quantity}</span>
      </Link>
    </li>
  ));
  return (
    <div className="shadow-lg w-full h-[40vh] p-4 overflow-auto scroll-bounce">
